...
the comment in `mise.toml`), so it is needed for `bun run test:e2e` and nothing
else.

`package.json` defines four: `format` (oxfmt), `lint` (oxlint), `types`
(tsc), `test` (bun test). Each checks without changing anything; `format:fix`
...
`bun run check` runs all four, `bun run ci` adds the e2e suite, and CI runs
those two by name. `mise.toml` pins the toolchain and nothing else — the
commands live in `package.json`, where every caller can read them.
...
assembles for itself. `bun run ci` and CI both run it, and so does anything
...
- `convex/_generated/server.{d.ts,js}` are regenerated by any push or deploy,
  including every `bun run test:e2e`. The committed files now match what the CLI
