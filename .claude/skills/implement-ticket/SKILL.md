...
2. **Scope check**: touch only what the ticket needs. If that includes a new dependency, a CI or `.claude/` rules change, or regenerating `src/routeTree.gen.ts`, do it — and give it its own line in **Notes for reviewer** saying why.
...
If you cannot finish — requirements ambiguous enough that two readings mean different work, or a failure you cannot legitimately fix:
