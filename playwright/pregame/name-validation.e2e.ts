...
test("join: an empty or disallowed name keeps the join button disabled", async ({ browser }) => {
  ...
  await guestPage.getByTestId(testIds.topic.nameInput).fill("Guest");
  await expect(guestPage.getByTestId(testIds.topic.submit)).toBeEnabled();

  await guestPage.getByTestId(testIds.topic.nameInput).fill("");
  await expect(guestPage.getByTestId(testIds.topic.submit)).toBeDisabled();

  await guestPage.getByTestId(testIds.topic.nameInput).fill("Bob!");
  await expect(guestPage.getByTestId(testIds.topic.submit)).toBeDisabled();
  await expect(
...
test("join: whitespace-only and duplicate names are accepted today", async ({ browser }) => {
  ...
  await blankPage.getByTestId(testIds.topic.nameInput).fill("   ");
  await expect(blankPage.getByTestId(testIds.topic.submit)).toBeEnabled();
  await blankPage.getByTestId(testIds.topic.submit).click();
  await expect(blankPage.getByTestId(testIds.lobby.leave)).toBeVisible();
  ...
  await twinPage.getByTestId(testIds.topic.nameInput).fill("Twin");
  await expect(twinPage.getByTestId(testIds.topic.submit)).toBeEnabled();
  await twinPage.getByTestId(testIds.topic.submit).click();
  await expect(twinPage.getByTestId(testIds.lobby.leave)).toBeVisible();
  await expect(twinPage.getByTestId(testIds.toast.root)).toHaveCount(0);

  await expect(hostPage.getByText("Twin", { exact: true })).toHaveCount(2);
  await expect(hostPage.getByTestId(testIds.lists.playerCount)).toHaveText("3 of 10");
